...
| `09_입시_제출` | 제출 전 검증, 시연 영상, 면접 준비 체크리스트 |
...
8. [입시 제출 체크리스트](09_입시_제출/입시_제출_체크리스트.md)
...
- `CompileAllBlueprints -ProjectOnly` 전체 블루프린트 컴파일은 0 errors / 0 failed load 상태로 정리했다.
- `BP_CheckpointBonfire`의 Material/MaterialInstance class mismatch 경고는 남아 있으나 현재 실행 차단 이슈는 아니다.
- 구형 `WBP_Inventory`는 C++ 인벤토리 UI로 대체되어 제거했다.
- `L_Runtime_Field`를 현재 테스트 기본 실행 맵으로 설정했다.
- CMD 보스방, Python 보스방, Vritra 보스방, Selvara 보스방을 데모용 크기로 확장 배치했다.
- CMD 왕좌, 보스방 안개문, 히든 무기 제단, 플레이어 무덤, Runtime Flask, Nel 기억 파편, 상징 나무, 체크포인트 에셋을 데모 맵에 배치했다.
- 기본 필드 / 보스방 바닥과 복도 / CMD 보스방 벽 텍스처를 머티리얼로 변환해 실제 맵 액터에 적용했다.
- 일반 필드 몬스터는 피격 경직, 공격 취소, 처치 경험치 지급을 지원한다.
- 체크포인트 휴식 메뉴에서 인벤토리, 저장, 레벨업 흐름을 연결했다.
- 레벨업은 남은 경험치를 소비해 원하는 스탯을 직접 올리는 구조이며, 사망 시 남은 경험치만 무덤에 드롭된다.
- 플레이어 무덤 회수 시 드롭된 경험치를 복구한다.
