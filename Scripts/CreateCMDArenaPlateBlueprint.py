...
BP_NAME = "BP_BossPlate3_CMDArena"
BP_PATH = f"{BP_DIR}/{BP_NAME}"
BP_CMD_PATH = "/Game/Blueprints/Bosses/BP_CMDBoss"


def log(message):
    unreal.log(f"[CreateCMDArenaPlateBlueprint] {message}")
...
def main():
    ...
    set_property_if_exists(cdo, "BossClassToSpawn", cmd_class)
    set_property_if_exists(cdo, "BossSpawnOffset", unreal.Vector(900.0, 0.0, 0.0))
    ...
    set_property_if_exists(cdo, "bPlayBossIntroBeforeAI", True)
    set_property_if_exists(cdo, "BossIntroDelay", 1.4)
